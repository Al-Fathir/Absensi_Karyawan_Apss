...
class Keluarga extends CI_Controller {
	...
	function get_all($offset = 0)
	{
		...
		// Load data
		$keluarga = $this->Keluarga_model->get_all($this->limit, $offset);
		...
		if ($num_rows > 0)
		{
			...
			$this->table->set_heading('No', 'NIK', 'Nama karyawan', 'Nama Keluarga', 'Jenis Kelamin' , 'Agama', 'Tempat / Tgl Lahir', 'Status', 'Actions');
			$i = 0 + $offset;
			
			foreach ($keluarga as $row)
			{
			$this->table->add_row(++$i, $row->nik, $row->nama_karyawan, $row->nama, $row->jenis_kelamin, $row->agama, $row->tempat_lahir.' / '.$row->tgl_lahir, $row->status, 
										anchor('keluarga/update/'.$row->nik,'update',array('class' => 'update')).' '.
										anchor('keluarga/delete/'.$row->nik,'hapus',array('class'=> 'delete','onclick'=>"return confirm('Anda yakin akan menghapus data ini?')"))
										);
			}
		...
		{
			$data['message'] = 'Tidak ditemukan satupun data keluarga!';
		}		
