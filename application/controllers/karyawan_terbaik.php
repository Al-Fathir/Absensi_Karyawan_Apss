...
class Karyawan_terbaik extends CI_Controller {
	/**
	 * Constructor
	 */
	...
	{
		parent::__construct();
		$this->load->model('Karyawan_terbaik_model', '', TRUE);
	}
	
	var $title = 'Karyawan Terbaik';
	...
	/**
	 * Menampilkan halaman utama rekap karyawan terbaik
	 */
	function main()
	{
		$data['title'] = $this->title;
		$data['h2_title'] = $this->title;
		$data['main_view'] = 'karyawan_terbaik/karyawan_terbaik';
		$data['form_action'] = site_url('karyawan_terbaik/get_karyawan_terbaik');
		
		$tahunini = date("Y");
		
		// data - data untuk dropdown menu		
		$departemen = $this->Karyawan_terbaik_model->get_departemen()->result();
	...
	function get_karyawan_terbaik()
	{
		$data['title'] = $this->title;
		$data['h2_title'] = $this->title;
		$data['main_view'] = 'karyawan_terbaik/karyawan_terbaik';
		$data['form_action'] = site_url('karyawan_terbaik/karyawan_terbaik');
		...
		$presensi = $this->Karyawan_terbaik_model->get_presensi($tanggalnya,$departemen)->result();
		$num_rows = $this->Karyawan_terbaik_model->get_presensi($tanggalnya,$departemen)->num_rows();
		...
		if ($num_rows > 0)
		{
			...
			$data['link'] = array('link_download' => anchor("karyawan_terbaik/downloadexcel/$departemennya/$tanggalnya",'download excel', array('class' => 'button'))
								);
	...
	function downloadexcel($departemen, $tanggalnya) {
		...
		$presensi = $this->Karyawan_terbaik_model->get_presensi($tanggalnya,$departemennya)->result();
		...
		$this->load->view('karyawan_terbaik/excel_view', $data);
